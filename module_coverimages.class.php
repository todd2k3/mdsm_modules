...
class module_coverimages extends module	{
	const version = 1.0;
	const moduleDesc = "Cover Images Module";
	...
	public function admin(){
		...
		if(isset($_REQUEST['mact'])){
			...
			$sSql = "SELECT imgID,imageLink,altText from repository_{$modID} ORDER by imgID ASC";
			$stm = $db->query($sSql);
			$imageList = "";
			if($stm){
				while($row = $stm->fetch(PDO::FETCH_ASSOC)){
					$imageList .= "<a href='{$row['imageLink']}' rel='lightbox' title='{$row['altText']}' />{$row['imageLink']}</a> <a href='javascript:void(0)' onclick=\"deleteImage({$row['imgID']})\">DELETE</a><br/>";
				}
			}
			if($imageList == ""){
...
<form method='post' id='formAdd' action='/admin/admin_ajax_modules.php' target='uploader' enctype='multipart/form-data'>
	...
	<fieldset>
		...
		<div><label>Alt Text:</label> <input type='text' name='altText'/><input id='saveImageButton' class='form_button' type='submit' value='Add'><input class='form_button' type='button' value='Help' onclick="getHelp('$modClass','module_coverimages');return false;"></div>
	...
	public function imageSave(){
		...
		if(isset($_FILES['image']['tmp_name'])){
			...
			if(is_numeric($nextID)){
				...
				$filename = $_SERVER['DOCUMENT_ROOT'] . "/media/cover-$modID-$nextID$file_ext";
				$vfilename = "/media/cover-$modID-$nextID$file_ext";
				if(move_uploaded_file($_FILES['image']['tmp_name'],$filename)){
                	if($_POST['altText'] == ""){$altText = "Cover Image $nextID";}else{$altText = $_POST['altText'];}
					$sSql = "INSERT INTO repository_$modID (imageLink,altText) VALUES ('$vfilename','$altText')";
					$db->exec($sSql);
                    $dbErr = $db->errorInfo();
					if($dbErr[2] == ""){
...
EOF;
    ...
    public function imageDelete(){
        ...
        $modID = parent::getModuleID();
    	$imgID = $_REQUEST['id'];
        $stm = $db->query("select imageLink from repository_$modID WHERE imgID = $imgID");
        $row = $stm->fetch(PDO::FETCH_ASSOC);
...
EOF;
            ...
            $sSql = "SELECT imageLink,altText from repository_$modID WHERE 1 = 1 LIMIT 10";
			$stm = $db->query($sSql);
            while($row = $stm->fetch(PDO::FETCH_ASSOC)){
            $altText = str_replace("'","\'",$row['altText']);
            	$images .= "<img class=\"jsslide\" src=\"{$row['imageLink']}\" alt=\"{$row['altText']}\" height=\"$imgHeight\" width=\"$imgWidth\" />";
            }
            return $images;
        }else{
			...
			$arrImgs = array();
			while($row = $stm->fetch(PDO::FETCH_ASSOC)){
				$arrImgs[] = array("path"=>$dp->makeDirty($row['imageLink']),"alt"=>$dp->makeDirty($row['altText']));
			}
			$selected = rand(0,count($arrImgs)-1);
			$image = $arrImgs[$selected];
			return "<img src='{$image['path']}' alt=\"{$image['alt']}\" />";
        }
	}
	...
	public function install(){
		...
		if($moduleID !== false){
			...
			$db->exec($sSql);
			$sSql = "CREATE TABLE repository_$moduleID (imgID int not null auto_increment primary key,imageLink varchar(150) not null,altText varchar(100))";
			$db->exec($sSql);
            $dbErr = $db->errorInfo();
			if($dbErr[2] != ""){
